...
Move Search::bestMove(Board* b, TimeManager* timeman, int threadId) {
    ...
    for (depth = 1; depth <= maxDepth; depth++) {
        ...
        for (td->pvIdx = 0; td->pvIdx < multiPv; ++td->pvIdx) {
            ...
            if (depth < 6) {
                score =
                    this->pvSearch(&searchBoard, -MAX_MATE_SCORE, MAX_MATE_SCORE, depth, 0, td, 0, 2);
                prevScore = score;
            } else {
            ...
            RootMove& curRootMove =
                *std::find(&td->rootMoves[0], &td->rootMoves[td->rootMoveCount], td->pv[0][0]);
            ...
            if (threadId == 0
                && (td->pvIdx + 1 == multiPv
                    || (depth > 1 && this->timeManager->elapsedTime() >= 3000))) {
                for (int pvLine = 0; pvLine < td->pvIdx + 1; ++pvLine) {
                    this->printInfoString(depth, td->rootMoves[pvLine].seldepth,
                                          td->rootMoves[pvLine].score, td->rootMoves[pvLine].pv,
                                          td->rootMoves[pvLine].pvLen, pvLine);
                }
                for (int pvLine = td->pvIdx + 1; pvLine < multiPv; ++pvLine) {
                    this->printInfoString(depth - 1, td->rootMoves[pvLine].seldepth,
                                          td->rootMoves[pvLine].prevScore, td->rootMoves[pvLine].pv,
                                          td->rootMoves[pvLine].pvLen, pvLine);
                }
                if (td->pvIdx == 0)
                    topScore = score;
...
Score Search::pvSearch(Board* b, Score alpha, Score beta, Depth depth, Depth ply, ThreadData* td,
    ...
    while ((m = mGen->next())) {
        ...
        if (ply == 0
            && std::find(&td->rootMoves[0], &td->rootMoves[td->pvIdx], m)
                   != &td->rootMoves[td->pvIdx])
            continue;
...
void Search::setMultiPv(int multiPvCount) { this->multiPvDefault = multiPvCount; }
void Search::stop() {
    if (timeManager)
        timeManager->stopSearch();
}
void Search::printInfoString(Depth depth, int sel_depth, Score score, Move* pv, uint16_t pvLen,
                             int pvIdx) {
    ...
    std::cout << "info"
              << " depth " << static_cast<int>(depth) << " seldepth " << sel_depth << " multipv "
              << (pvIdx + 1);
